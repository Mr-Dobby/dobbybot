...
module.exports.run = async (bot, message, args, client) => {
  ...
  let currPrefix = await Servers.findOne( { guildID: message.guild.id } )
    ...
    let OnlyStaff = new Discord.RichEmbed()
        .setAuthor(`${message.author.tag} | Remove Member from Ticket`, message.author.displayAvatarURL)
    ...
    let TicketEmbedLog = new Discord.RichEmbed()
        .setAuthor(`${message.author.tag} | Removed Member from Ticket 🔒`, message.author.displayAvatarURL)
        .setDescription(`\`${currPrefix.prefix}remove <Ticket ID> <User ID>\``)
        .setColor("#ffc500")
        .addField("Removed member from ticket", `${member.user.tag}\n<@${member.user.id}>`, true)
    ...
    let AMTicketEmbedSuccess = new Discord.RichEmbed()
        .setAuthor(`${message.author.tag} | Add Member from Ticket`, message.author.displayAvatarURL)
        .setDescription(`${Sucess} <@${message.author.id}> removed <@${member.id}> from this ticket!`)
        .setColor("#ffc500")
...
module.exports.help = {
  name: "remove",
  aliases: ["rmticket"] //{Remove} {Member} (to) {Ticket} => amticket
}
