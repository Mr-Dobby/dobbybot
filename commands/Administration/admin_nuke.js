...
module.exports.run = async (bot, message, args, client) => {
    ...
    let NukePrep = new Discord.RichEmbed()
        .setColor("#ff0000")
    ...
    while (!statusMsg2) {
            ...
            if (!msgs.size || !['y', 'yes'].includes(msgs.first().content.toLowerCase())) return message.channel.send(CancelledEmbed).then(message => message.delete(5000));
            if (['n', 'no'].includes(msgs.first().content.toLowerCase())) return message.channel.send(CancelledEmbed).then(message => message.delete(5000));
    ...
    var nukeLog = new Discord.RichEmbed()
